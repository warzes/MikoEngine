...
layout(location = 1) in  vec2  TexCoord;	// 32 bit texture coordinate
layout(location = 1) out vec2  TexCoordVs;	// Texture coordinate
layout(location = 2) in  ivec4 QTangent;	// 16 bit QTangent
layout(location = 2) out vec3  TangentVs;	// Tangent space to view space, x-axis
layout(location = 3) out vec3  BinormalVs;	// Tangent space to view space, y-axis
...
void main()
{
	...
	vec4 value_argb_nxa = texture(_argb_nxa, TexCoordVs);
	vec4 value_hr_rg_mb_nya = texture(_hr_rg_mb_nya, TexCoordVs);
