...
bool Instancing::init(int argc, const char * argv[])
{
...
#include "Instancing_InstancedArrays_GLSL_450.h"	// For Vulkan
#include "Instancing_InstancedArrays_GLSL_410.h"	// macOS 10.11 only supports OpenGL 4.1 hence it's our OpenGL minimum
#include "Instancing_InstancedArrays_GLSL_ES3.h"
#include "Instancing_InstancedArrays_HLSL_D3D11_D3D12.h"
#include "Instancing_InstancedArrays_Null.h"
...
#include "Instancing_DrawInstanced_HLSL_D3D11_D3D12.h"
...
void Instancing::update(double delta)
{
...
ApplicationSetting Instancing::intial_app_settings()
{
...
void Instancing::window_resized(int width, int height)
{
}
...
void Instancing::fillCommandBuffer()
{
	...
	SE_ASSERT(nullptr != mGraphicsPipelineStateInstancedArrays, "Invalid graphics pipeline state instanced arrays");
	SE_ASSERT(nullptr != mVertexArrayInstancedArrays, "Invalid vertex array instanced arrays");
	SE_ASSERT(!rhi->getCapabilities().drawInstanced || nullptr != mGraphicsPipelineStateDrawInstanced, "Invalid graphics pipeline state draw instanced");
	SE_ASSERT( !rhi->getCapabilities().drawInstanced || nullptr != mVertexArrayDrawInstanced, "Invalid vertex array draw instanced");
