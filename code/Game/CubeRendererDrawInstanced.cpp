	...
	mBatches(nullptr)
{
	...
	{ // Create the textures
		...
		static constexpr uint32_t TEXTURE_WIDTH = 1440;
		static constexpr uint32_t TEXTURE_HEIGHT = 900;
		...
		{ // Fill the texture content
			...
			int height = 128;
			int bitdepth = 4;			
			uint8_t* dataTex = stbi_load("timg.jpg", &width, &height, &bitdepth, 0);
			uint8_t* RESTRICT dataCurrentTex = dataTex;

			for ( uint32_t j = 0; j < mNumberOfTextures; ++j )
			{
				dataCurrentTex = dataTex;
				
				for ( uint32_t i = 0; i < TEXTURE_WIDTH * TEXTURE_HEIGHT; ++i )
				{
					*dataCurrent = *dataCurrentTex;
					++dataCurrent;
					++dataCurrentTex;
					*dataCurrent = *dataCurrentTex;
					++dataCurrent;
...
#include "CubeRendererDrawInstanced_HLSL_D3D11_D3D12.h"
...
void CubeRendererDrawInstanced::setNumberOfCubes(uint32_t numberOfCubes)
{
	...
	{
		const uint32_t currentNumberOfCubes = (remaningNumberOfCubes > static_cast<int>(mMaximumNumberOfInstancesPerBatch)) ? mMaximumNumberOfInstancesPerBatch : remaningNumberOfCubes;
		// TODO: отключил полупрозрачность
...
void CubeRendererDrawInstanced::fillReusableCommandBuffer()
{
	...
	RHI_ASSERT(nullptr != mTexture2DArray, "Invalid texture 2D array");
