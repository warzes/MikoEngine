...
#include <Renderer/Resource/CompositorNode/Pass/ShadowMap/CompositorInstancePassShadowMap.h>
...
Scene::Scene() :
	...
	mFirstFrame(true),
	mSceneResourceId(GetInvalid<Renderer::SceneResourceId>()),
	mMaterialResourceId(GetInvalid<Renderer::MaterialResourceId>()),
	mCloneMaterialResourceId(GetInvalid<Renderer::MaterialResourceId>()),
...
bool Scene::init(int argc, const char * argv[])
{
	...
	if ( Rhi::NameId::VULKAN == nameId || Rhi::NameId::DIRECT3D12 == nameId )
	{
...
ApplicationSetting Scene::intial_app_settings()
{
...
void Scene::window_resized(int width, int height)
{
}

Renderer::IRenderer* Scene::getRenderer() const
{
...
void Scene::onLoadingStateChange(const Renderer::IResource& resource)
{
	...
	else if ( Renderer::IResource::LoadingState::LOADED == loadingState && resource.getAssetId() == ::detail::IMROD_MATERIAL_ASSET_ID )
	{
		...
		mCloneMaterialResourceId = getRenderer()->getMaterialResourceManager().createMaterialResourceByCloning(resource.getId());
		trySetCustomMaterialResource();
	}
}
...
void Scene::loadIni()
{
	...
	{
		IFile* file = fileManager.openFile(IFileManager::FileMode::READ, ::detail::VIRTUAL_SETTINGS_FILENAME);
		if ( nullptr != file )
		{
...
void Scene::saveIni()
{
	if ( nullptr != mIni )
	{
		const IFileManager& fileManager = getRenderer()->getFileManager();
		IFile* file = fileManager.openFile(IFileManager::FileMode::WRITE, ::detail::VIRTUAL_SETTINGS_FILENAME);
		if ( nullptr != file )
		{
			Renderer::IRenderer& renderer = *getRenderer();
			char temp[256];
...
{
	if ( nullptr != mCompositorWorkspaceInstance && IsValid(mSceneResourceId) )
	{
...
{
	if ( !mCustomMaterialResourceSet && nullptr != mSceneNode && IsValid(mCloneMaterialResourceId) )
	{
		for ( Renderer::ISceneItem* sceneItem : mSceneNode->getAttachedSceneItems() )
		{
			if ( sceneItem->getSceneItemTypeId() == Renderer::MeshSceneItem::TYPE_ID )
			{
				...
				if ( Renderer::IResource::LoadingState::LOADED == getRenderer()->getMeshResourceManager().getResourceByResourceId(meshSceneItem->getMeshResourceId()).getLoadingState() )
				{
