...
constexpr const char* g_sample_vs_src = R"(
...
constexpr const char* g_sample_fs_src = R"(
...
public:
    ...
    ApplicationSetting intial_app_settings() override
    {
        ...
        settings.width = 1280;
        settings.height = 720;
        settings.title = "Hello Game (OpenGL)";
...
private:
    ...
    {
        // Create shaders
        m_vs = std::make_unique<gl::Shader>(GL_VERTEX_SHADER, g_sample_vs_src);
        m_fs = std::make_unique<gl::Shader>(GL_FRAGMENT_SHADER, g_sample_fs_src);
        ...
        gl::Shader* ezshaders[] = { m_vs.get(), m_fs.get() };
        m_program = std::make_unique<gl::Program>(2, ezshaders);

        if ( !m_program )
        {
            SE_LOG_FATAL("Failed to create Shader Program");
            return false;
        }
    ...
    bool create_uniform_buffer()
    {
        ...
        m_ubo = std::make_unique<gl::UniformBuffer>(GL_DYNAMIC_DRAW,
            sizeof(Transforms),
            nullptr);
    ...
    {
        SE_SCOPED_SAMPLE("update_uniforms");
...
private:
    // GPU resources.
    std::unique_ptr<gl::Shader>        m_vs;
    std::unique_ptr<gl::Shader>        m_fs;
