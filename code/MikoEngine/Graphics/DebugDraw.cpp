...
)
{
	...
	{
		SE_LOG_FATAL("Failed to create Shaders");
		return false;
	}
	...
	{
		SE_LOG_FATAL("Failed to create Vertex Buffers/Arrays");
		return false;
	}
...
void DebugDraw::render(vk::Backend::Ptr backend, vk::CommandBuffer::Ptr cmd_buffer, int width, int height, const glm::mat4& view_proj, const glm::vec3& view_pos)
{
	if ( m_world_vertices.size() > 0 )
	{
		...
		if ( m_world_vertices.size() > MAX_VERTICES )
			SE_LOG_ERROR("Vertex count above allowed limit!");
		else
...
void DebugDraw::create_descriptor_set_layout(vk::Backend::Ptr backend)
{
	...
	m_ds_layout = vk::DescriptorSetLayout::create(backend, desc);
}
...
void DebugDraw::create_uniform_buffer(vk::Backend::Ptr backend)
{
	...
	m_ubo = vk::Buffer::create(backend, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, m_ubo_size * vk::Backend::kMaxFramesInFlight, VMA_MEMORY_USAGE_CPU_TO_GPU, VMA_ALLOCATION_CREATE_MAPPED_BIT);
}
...
void DebugDraw::create_vertex_buffer(vk::Backend::Ptr backend)
{
	...
	m_line_vbo = vk::Buffer::create(backend, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, m_vbo_size * vk::Backend::kMaxFramesInFlight, VMA_MEMORY_USAGE_CPU_TO_GPU, VMA_ALLOCATION_CREATE_MAPPED_BIT);
}
...
void DebugDraw::create_pipeline_states(vk::Backend::Ptr backend, vk::RenderPass::Ptr render_pass)
{
	...
	vk::ShaderModule::Ptr vs = vk::ShaderModule::create(backend, spirv);

	spirv.resize(kDebugDrawFragSPV_size);
	memcpy(&spirv[0], &kDebugDrawFragSPV_data[0], kDebugDrawFragSPV_size);

	vk::ShaderModule::Ptr fs = vk::ShaderModule::create(backend, spirv);
	...
	m_pipeline_layout = vk::PipelineLayout::create(backend, pl_desc);
	...
	m_line_depth_pipeline = vk::GraphicsPipeline::create(backend, pso_desc);
	...
	m_line_no_depth_pipeline = vk::GraphicsPipeline::create(backend, pso_desc);
	...
	m_line_strip_depth_pipeline = vk::GraphicsPipeline::create(backend, pso_desc);
	...
	m_line_strip_no_depth_pipeline = vk::GraphicsPipeline::create(backend, pso_desc);
}
#endif