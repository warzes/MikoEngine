...
//[ Includes                                              ]
//[-------------------------------------------------------]
#include "Renderer/Resource/ShaderBlueprint/Cache/ShaderBuilder.h"
	...
	namespace detail
	{
		...
		bool findBlockEnd(const Rhi::Context& context, SubStringRef& outSubString, bool& syntaxError, bool allowsElse = false)
		{
				...
				if ('@' == *it)
				{
					...
					else
					{
						...
						if (0 == idx)
						{
							...
							{
								syntaxError = true;
								RHI_LOG(CRITICAL, "Unexpected @else while looking for @end\nNear: \"%s\"\n", &(*subString.begin()))
							}
							if (0 == nesting)
							{
			...
			else
			{
				...
				RHI_LOG(CRITICAL, "Syntax error at line %lu: Start block (e.g. @foreach; @property) without matching @end\nNear: \"%s\"\n", calculateLineCount(outSubString), tmpData)
			}
		...
		[[nodiscard]] size_t evaluateExpressionEnd(const Rhi::Context& context, const SubStringRef& outSubString)
		{
			...
			{
				RHI_LOG(CRITICAL, "Renderer shader builder: Syntax error at line %lu: Opening parenthesis without matching closure\n", static_cast<unsigned long>(calculateLineCount(outSubString)))
			}
		...
		[[nodiscard]] bool evaluateExpressionRecursive(const Rhi::Context& context, const Renderer::ShaderProperties& shaderProperties, ExpressionVec& expression, bool& outSyntaxError)
		{
			...
			while (itor != end && !syntaxError)
			{
				...
				{
					syntaxError = true;
					RHI_LOG(CRITICAL, "Renderer shader builder: Unrecognized token '%s'", exp.value.c_str())
				}
		...
		[[nodiscard]] bool evaluateExpression(const Rhi::Context& context, const Renderer::ShaderProperties& shaderProperties, SubStringRef& outSubString, bool& outSyntaxError)
		{
			...
			if (syntaxError)
			{
				RHI_LOG(CRITICAL, "Renderer shader builder: Syntax error at line %lu\n", static_cast<unsigned long>(calculateLineCount(subString)))
			}
			outSyntaxError = syntaxError;
			return retVal;
		}
		...
		void evaluateParamArgs(const Rhi::Context& context, SubStringRef& outSubString, StringVector& outArgs, bool& outSyntaxError)
		{
			...
			while (it != en && !syntaxError)
			{
				...
				else
				{
					...
					{
						RHI_LOG(CRITICAL, "Renderer shader builder: Syntax Error at line %lu: ',' or ')' expected\n", static_cast<unsigned long>(calculateLineCount(subString)))
						syntaxError = true;
					}
					else
					{
...
namespace Renderer
{
	...
	bool ShaderBuilder::parseMath(const std::string& inBuffer, std::string& outBuffer)
	{
		...
		while (std::string::npos != pos && !syntaxError)
		{
			...
			if (syntaxError)
			{
				...
				if (keyword <= 1)
				{
					RHI_LOG(CRITICAL, "Renderer shader builder: Syntax error at line %lu: @%s expects one parameter", lineCount, ::detail::c_operations[keyword].opName)
				}
				else
				{
					RHI_LOG(CRITICAL, "Renderer shader builder: Syntax error at line %lu: @%s expects two or three parameters", lineCount, ::detail::c_operations[keyword].opName)
				}
			}
			else
			{
			...
			if (!syntaxError)
			{
				...
				if (argValues.size() > 2)
				{
					...
					if (argValues[2].c_str() == endPtr)
					{
						...
						{
							RHI_LOG(CRITICAL, "Renderer shader builder: Invalid parameter at line %lu (@foreach). '%s' is not a number nor a variable\n", static_cast<unsigned long>(calculateLineCount(blockSubString)), argValues[2].c_str())
							syntaxError = true;
							start = 0;
							count = 0;
						}
	...
	bool ShaderBuilder::collectPieces(const std::string& inBuffer, std::string& outBuffer)
	{
		...
		while (std::string::npos != pos && !syntaxError)
		{
			...
			if (syntaxError)
			{
				RHI_LOG(CRITICAL, "Renderer shader builder: Syntax error at line %lu: @piece expects one parameter", static_cast<unsigned long>(calculateLineCount(subString)))
			}
			else
			{
				...
				{
					syntaxError = true;
					RHI_LOG(CRITICAL, "Renderer shader builder: Error at line %lu: @piece '%s' already defined", static_cast<unsigned long>(calculateLineCount(subString)), argValues[0].c_str())
				}
				else
				{
					::detail::SubStringRef blockSubString = subString;
	...
	bool ShaderBuilder::insertPieces(std::string& inBuffer, std::string& outBuffer) const
	{
		...
		while (std::string::npos != pos && !syntaxError)
		{
			...
			if (syntaxError)
			{
				RHI_LOG(CRITICAL, "Renderer shader builder: Syntax error at line %lu: @insertpiece expects one parameter", static_cast<unsigned long>(calculateLineCount(subString)))
			}
			else
			{
				const StringId pieceName(argValues[0].c_str());
				...
				{
					RHI_LOG(CRITICAL, "Renderer shader builder: Error at line %lu: @insertpiece is referencing unknown piece \"%s\"", static_cast<unsigned long>(calculateLineCount(subString)), argValues[0].c_str())
				}
			}
	...
	bool ShaderBuilder::parseCounter(const std::string& inBuffer, std::string& outBuffer)
	{
		...
		while (std::string::npos != pos && !syntaxError)
		{
			...
			if (syntaxError)
			{
				...
				if (keyword <= 1)
				{
					RHI_LOG(CRITICAL, "Renderer shader builder: Syntax error at line %lu: @%s expects one parameter", lineCount, ::detail::c_counterOperations[keyword].opName)
				}
				else
				{
					RHI_LOG(CRITICAL, "Renderer shader builder: Syntax error at line %lu: @%s expects two or three parameters", lineCount, ::detail::c_counterOperations[keyword].opName)
				}
			}
			else
			{
