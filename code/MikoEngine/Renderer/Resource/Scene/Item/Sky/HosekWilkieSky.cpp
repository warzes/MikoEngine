...
namespace
{
	namespace detail
	{
		...
		//[ Global variables                                      ]
		//[-------------------------------------------------------]
		#include "Renderer/Resource/Scene/Item/Sky/ArHosekSkyModelData_RGB.h"
		...
		void compute(const glm::vec3& worldSpaceSunDirection, float turbidity, float albedo, float normalizedSunY, Renderer::HosekWilkieSky::Coefficients& coefficients)
		{
	...
	void HosekWilkieSky::recalculate(const glm::vec3& worldSpaceSunDirection, float turbidity, float albedo, float normalizedSunY)
	{
		if (mWorldSpaceSunDirection != worldSpaceSunDirection || mTurbidity != turbidity || mAlbedo != albedo || mNormalizedSunY != normalizedSunY)
		{
			...
			if (worldSpaceSunDirection.y > 0.0f)
			{
				...
				mSunColor = glm::vec3(mSunColor.z, mSunColor.y, mSunColor.x);
			}
		}
	}
