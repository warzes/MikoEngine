...
void Scene::add_instance(Mesh::Ptr mesh, glm::mat4 transform)
{
...
void Scene::create_descriptor_sets(vk::Backend::Ptr backend, bool ray_tracing)
{
	...
	if ( ray_tracing )
	{
		...
		m_ray_tracing_geometry_ds_layout = vk::DescriptorSetLayout::create(backend, desc);
	...
	SE_ZERO_MEMORY(set_layout_binding_flags);
	...
	m_material_ds_layout = vk::DescriptorSetLayout::create(backend, desc);
	...
	if ( ray_tracing )
	{
		...
		SE_ZERO_MEMORY(geometry_write_data[0]);
		SE_ZERO_MEMORY(geometry_write_data[1]);
		SE_ZERO_MEMORY(geometry_write_data[2]);
	...
	SE_ZERO_MEMORY(material_write_data[0]);
	SE_ZERO_MEMORY(material_write_data[1]);
	SE_ZERO_MEMORY(material_write_data[2]);
	SE_ZERO_MEMORY(material_write_data[3]);
